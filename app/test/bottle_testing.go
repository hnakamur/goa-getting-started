...
func ShowBottleNotFound(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.BottleController, bottleID int) http.ResponseWriter {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
...
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowBottleOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.BottleController, bottleID int) (http.ResponseWriter, *app.GoaMyexampleBottle) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	var mt *app.GoaMyexampleBottle
	if resp != nil {
		var ok bool
		mt, ok = resp.(*app.GoaMyexampleBottle)
		if !ok {
			t.Fatalf("invalid response media: got %+v, expected instance of app.GoaMyexampleBottle", resp)
		}
		err = mt.Validate()
		if err != nil {
